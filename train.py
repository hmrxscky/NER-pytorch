...
def evaluating(model, datas, best_F):
    ...
    for line in prediction:
        ...
        if len(parts) == 3:
            ...
            pred_tags.append(parts[2])

    from sklearn.metrics import precision_recall_fscore_support
    labels = [tag for tag in tag_to_id.keys() if tag != 'O']
    p, r, f1, _ = precision_recall_fscore_support(
        true_tags, pred_tags, labels=labels, average='micro', zero_division=0
    )
    new_F = f1 * 100 
...
for epoch in range(1, 10001):
    ...
    for i, index in enumerate(np.random.permutation(len(train_data))):
        ...
        count += 1
        if count >= 42123:
            print('Finished 3 epochs, stopping training.')
            break
...
plt.figure()
plt.plot(all_F_arr[:, 0], label='train')
plt.plot(all_F_arr[:, 1], label='dev')
plt.plot(all_F_arr[:, 2], label='test')
plt.title('F1 Score')
plt.xlabel('Evaluation Steps (x20)')
plt.ylabel('F1')
plt.legend()
